...
import com.novabank.mscuentas.dto.response.ErrorResponse;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleFeignNotFound(
        ...
        log.warn("Recurso no encontrado en ms-clientes");

        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(ErrorResponse.of(400, "Bad Request",
                        "Cliente no encontrado en ms-clientes", request.getRequestURI()));
    }
    ...
    public ResponseEntity<ErrorResponse> handleFeign(
        ...
        log.error("Error invocando ms-clientes: status={}", ex.status());

        return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
                .body(ErrorResponse.of(503, "Service Unavailable",
                        "ms-clientes no disponible", request.getRequestURI()));
    }
